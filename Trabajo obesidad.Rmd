...
A continuación, mostramos histogramas de todas las variables continuas, aunque mas adelante analizaremos más detalladamente algunos de ellos.
...
Calculando la estadísticas descriptivas de las variables númericas obtenemos un resumen numérico de las características de los datos:
...
La mayoría de personas tienen entre 15 y 30 años, con un pico en los 20 años, por lo que los datos que estamos analizando son mayoritariamente de personas jóvenes. A partir de los 30 años la frecuencia disminuye con muy pocos casos por encima de los 50 años.

```{r}
train |> ggplot(aes(Height)) +
  geom_histogram(
    fill = "pink", 
    bins = 15,           
    col = "white"        
...
Muchas más personas tienen un historial familiar de sobrepeso de las que no lo tienen, intuitivamente podríamos decir que esto afecta directamente al nivel de obesidad de la persona asi que más adelante analizaremos esta variable en función de los niveles de obesidad para ver si la suposición es cierta.

Los medios de transporte utilizados por las personas también pueden afectar al nivel de obesidad ya que pueden suponer más o menos actividad física, veremos la frecuencia de cada uno para más adelante analizarlos en función de los niveles de obesidad:
...
Ahora, nos centraremos más en el target y en ver como se relaciona con el resto de variables de distintas formas.
...
train |> ggplot(mapping = aes(x = Height, y = Weight, color = NObeyesdad)) +
  geom_point()
```
Hay una clara tendencia positiva, a mayor altura, mayor peso. Los diferentes niveles de obesidad se distribuyen de forma ascendente, con el peso aumentando progresivamente, lo que nos confirma que el peso es clave para clasificar el nivel de obesidad.
...
Podemos ver que las personas que hacen una comida son menos propensos de desarrollar obesidad de algún tipo. Por otro lado, hay una cosa que nos parece curiosa y es que al hacer tres comidas hay bastantes personas que han desarrollado obesidad de tipo 3 o no tienen el suficiene peso. Viendo esta tabla se puede ver que las personas hacen sobretodo 3 comidas al día.
...
Viendo esta tabla podemos ver que si las personas fuman no tiene una relación con el nivel de obesidad. Debido a que la mayoría de las personas que fuman se encuentran en obesidad de tipo II, sin embargo el siguiente grupo en el que las personas fuman son personas de peso normal. 
...
```{r}
train_num <- train %>% dplyr::select(Age, Height, Weight, FCVC, NCP, CH2O, FAF, TUE)
