...
Continuamos evaluando visualmente la convergencia del modelo, para analizar el comportamiento del modelo según se agregan árboles al conjunto.
...
## Análisis en el conjunto de validación del modelo Random Forest
...
Evaluando el modelo en validación este alcanza un rendimiento excelente con unos valores de accuracy y Kappa que superan incluso los resultados obtenido en test.
...
La sensibilidad media es muy alta también, con un valor de 0,95, lo cual indica que el modelo identifica bien la mayoría de los casos en todas las clases.
La especificidad también destaca con un 99,4%, mostrando que el modelo casi no comete falsos positivos.

Comparado con los resultados en test, donde Random Forest ya era el más exacto, al evaluar en validación se ha reafirmado su fiabilidad. Es claramente el más consistente frente a otros modelos, por lo que demuestra ser el modelo con el mejor rendimiento. 

Además, ofrece interpretabilidad mediante la importancia de variables, donde destacan el peso, la altura y la edad, lo que refuerza por tanto la coherencia del modelo sobre que estas variables son cruciales para clasificar los niveles de obesidad.


## Conclusiones y propuestas de mejoras

Tras analizar y modelar el conjunto de datos de obesidad, aplicar diversas técnicas, abordar distintos modelos y evaluar su rendimiento con distintas métricas, hemos podido sacar conclusiones sobre ellos. 

Los resultados a lo largo del trabajo han mostrado que las variables más determinantes para clasificar la edad son el peso, la altura y la edad, lo que concuerda con los conocimientos médicos acerca de la obesidad de hoy en dia. 

También hemos podido detectar como algunas clases de obesidad como Sobre Peso Nivel I y Nivel II han presentado mas dificultades para clasificarse, por ser mas cercanas, mientras que Obesdiad de tipo III y Peso insuficiente se identifican con mucha más facilidad. 

En definitiva, tras aplicar todos estos modelos, técnicas y compararlas, destacando Random Forest que nos permite obtener las predicciones más precisas y generalizables, los resultados obtenidos han sido útiles y nos han permitido sacar conclusiones acerca de la obesidad que podrían aplicarse en entornos reales.

Algunas mejoras que podríamos añadir sería, si se pudiese, ampliar el número de variables (añadir horas de sueño, trabajo, etc) que podría sernos de bastante utilidad para así hacer un estudio más en profundidad, poder  captar aún mejor la complejidad de la obesidad


## Trabajo a Futuro

Desarrollar modelos que incorporen series temporales de peso y hábitos ((por ejemplo, mediante Redes Neuronales) para predecir trayectorias de riesgo de obesidad a lo largo del tiempo.

Implementación de un prototipo de herramienta web o aplicación móvil que permita la carga de datos por parte de profesionales sanitarios y ofrezca predicciones y recomendaciones inmediatas a los usuarios.

Establecer un proceso de reentrenamiento a medida que se recolecten nuevos datos, para mantener la precisión y adaptabilidad de los modelos a cambios en la población o en los hábitos de vida y así que nuestro modelo pueda no acabar obsoleto y siga recibiendo estas actualizaciones de datos de pacientes y usuarios para mejorar el modelo y que siga siendo útil para la sociedad.

Colaboración con profesionales sanitarios (médicos, nutricionistas o dietistas) y así poder incorporar su feedback para ajustar las características (por ejemplo, nuevos indicadores clínicos) y validar que las predicciones sean útiles y realistas en la práctica. Con esto también podríamos organizar sesiones de revisión periódicas para asegurar que el modelo evolucione acorde a los avances médicos y las necesidades de los pacientes consiguiendo de esta forma un mayor impacto real.


## Division del trabajo
El análisis de los modelos k-NN y lda, la creación de esoso dos modelos y la interpretación de las medidas de rendimeinto de esoso dos modelos han sido desarrollados por Marcos López.
Los modelos bagging y Naive Bayes han sido desarrollados por Carmen Liberal. 
Los modelos Boosting, Arboles de Decisión, Random Forest desarrollados por Lara Montero.
La comparación de modelos ha sido realizada por Lara Montero. Las conclusiones sobre validación, conclusion final y trabajo a futuro han sido realizadas por Carmen Liberal y Lara Montero.
