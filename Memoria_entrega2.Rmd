...
```{r}
train$y    <- as.numeric(as.factor(train$NObeyesdad)) - 1
test$y     <- as.numeric(factor(test$NObeyesdad, 
                                levels = levels(as.factor(train$NObeyesdad)))) - 1
levels_lbl <- levels(as.factor(train$NObeyesdad))
K          <- length(levels_lbl)

pred_cols <- setdiff(names(train), c("NObeyesdad", "y"))
train[pred_cols] <- lapply(train[pred_cols], function(z) 
  if(is.numeric(z)) z else as.numeric(as.factor(z)))
test[pred_cols]  <- lapply(test[pred_cols], function(z) 
  if(is.numeric(z)) z else as.numeric(as.factor(z)))

df_train <- as.data.frame(train[pred_cols])
df_train$y <- factor(train$y, levels = 0:(K-1))
df_test  <- as.data.frame(test[pred_cols])

adaboost_train_rpart <- function(df, M = 10) {
  n      <- nrow(df)
  w      <- rep(1/n, n)
  ...
  for (m in seq_len(M)) {
    stump <- rpart(y ~ ., data = df, weights = w,
...
adaboost_predict_rpart <- function(model, df) {
  ...
  for (m in seq_len(M)) {
    ...
    pred_num    <- as.numeric(pred_factor) - 1
    for (k in 0:(model$K - 1)) {
      scores[, k+1] <- scores[, k+1] + model$alphas[m] * (pred_num == k)
    }
  }
  apply(scores, 1, which.max) - 1
}

# 5) Medir tiempo, entrenar y evaluar
time_taken <- system.time({
  model_ab <- adaboost_train_rpart(df_train, M = 10)
})
cat(sprintf("Tiempo: %.2f s\n", time_taken[3]))

preds    <- adaboost_predict_rpart(model_ab, df_test)
accuracy <- mean(preds == test$y) * 100
cat(sprintf("Accuracy en test: %.2f%%\n", accuracy))
```

En conclusión, el algoritmo implementado es mucho más eficiente (número de Accuracy de 40% aproximadamente) ya que las iteraciones y bucles son controladas, el propio algoritmo está personalizado a nuestro caso concreto y hemos ajustado los parámetros para nuestro ejemplo concreto.
