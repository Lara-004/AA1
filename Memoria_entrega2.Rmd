...
```{r}
source("Memoria_Funciones.Rmd")

train.df_x <- train.df |> dplyr::select(is.numeric)
test.df_x <- test.df |> dplyr::select(is.numeric)
...
```{r}
df.test <- test %>%
  dplyr::select(Gender, Age, Height, Weight,
                family_history_with_overweight, FAVC, FCVC, NCP,
                CAEC, SMOKE, SCC, FAF, TUE, CH2O, CALC, MTRANS)
prediction.rf <- predict(rf, df.test)

niveles_comunes <- union(levels(prediction.rf), levels(test$NObeyesdad))

prediction.rf <- factor(prediction.rf, levels = niveles_comunes)
real.rf <- factor(test$NObeyesdad, levels = niveles_comunes)
