...
```{r}
# ——— DEBUG: compara clases de cada columna ———
...
# ——— DEBUG: comprueba niveles de los factores ———
...
# Para cada factor, imprime niveles en train vs test
for(v in factor_vars) {
  cat("Variable:", v, "\n")
  cat("  Niveles en train:", paste(levels(df.train[[v]]), collapse = ", "), "\n")
  cat("  Niveles en test: ", paste(levels(df.test[[v]]),  collapse = ", "), "\n\n")
}

# ——— DEBUG: busca NAs introducidos por re-factoring ———
na_counts <- sapply(df.test, function(x) sum(is.na(x)))
print(data.frame(variable = names(na_counts), nas = na_counts))
...
# Verifica que ya coincidan
#mismatch <- sapply(names(df.train), function(v) {
#  paste(class(df.train[[v]]), collapse = "/") != paste(class(df.test[[v]]), collapse="/")
#})
#if (any(mismatch)) {
#  warning("Siguen sin coincidir clases en: ", paste(names(mismatch)[mismatch], collapse=", "))
#}
